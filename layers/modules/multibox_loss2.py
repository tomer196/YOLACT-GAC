...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, targets, masks, num_crowds, dist_maps):
        """Multibox Loss
        Args:
        ...
        for idx in range(batch_size):
            ...
            if cur_crowds > 0:
                ...
                _, dist_maps[idx]  = split(dist_maps[idx])
            else:
                crowd_boxes = None
        ...
        if cfg.train_masks:
            if cfg.mask_type == mask_type.direct:
                ...
                ret = self.lincomb_mask_loss(pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, dist_maps)
                if cfg.use_maskiou:
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, dist_maps, interpolation_mode='bilinear'):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                ###
                downsampled_dist_maps = F.interpolate(dist_maps[idx].unsqueeze(0), (mask_h, mask_w),
                                                  mode=interpolation_mode, align_corners=False).squeeze(0)
                downsampled_dist_maps = downsampled_dist_maps.permute(1, 2, 0).contiguous()
